...
//class SayV1 implements OrchestratorTelegramInterface.IClass {
...
//		const generate = await modules("Inference").invoke.getPromt(text, history);
//
//		console.log(text + "\n", generate);
//
//		await modules("History").invoke.setHistory(chatId, new Date().getTime(), text, generate);
//
//		const chunks = generate!.match(/[\s\S]{1,4000}(?=\s|$)/g) || [];
