...
class PaymentImpTest implements PaymentInterface.IAdapter {
	...
	async createPayAddress() {
		try {
			if (Math.round(Math.random())) throw "createPayAddress";
	...
	async checkBalanceUsdt(address: string) {
		try {
			if (Math.round(Math.random())) throw "checkBalanceUsdt";
	...
	async isExistTransaction(address: string, min_timestamp: string, sum: string) {
		try {
			if (Math.round(Math.random())) throw "isExistTransaction";
	...
	async checkBalanceTrx(address: string) {
		try {
			if (Math.round(Math.random())) throw "checkBalanceTrx";
	...
	async fundingAddress(address: string, amount: number) {
		try {
			if (Math.round(Math.random())) throw "fundingAddress";
	...
	async sendUsdtWallet(privateKey: string, toAddress: string, amount: number) {
		try {
			if (Math.round(Math.random())) throw "sendUsdtWallet";
